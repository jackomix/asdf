...
/* Android arm64 uses one 64-bit word for sigset_t, while glibc uses 128
 * bytes.  Passing a guest sigset_t to the host functions overwrites adjacent
 * Android objects (including nativeRecreateGfxState's saved return address).
 * Keep these entry points separate so this file can still use host sigset_t
 * internally for sigaction and pthread operations. */
typedef unsigned long ter_bionic_sigset_t;
_Static_assert(sizeof(ter_bionic_sigset_t) == 8,
               "Terraria requires the arm64 bionic sigset_t ABI");

static void ter_bionic_sigset_to_host(ter_bionic_sigset_t bset,
                                      sigset_t *host) {
  sigemptyset(host);
...
int ter_bionic_sigemptyset(ter_bionic_sigset_t *set) {
  if (!set) { errno = EINVAL; return -1; }
  *set = 0;
  return 0;
}

int ter_bionic_sigfillset(ter_bionic_sigset_t *set) {
  if (!set) { errno = EINVAL; return -1; }
  *set = ~0UL;
  return 0;
}

int ter_bionic_sigaddset(ter_bionic_sigset_t *set, int signal_number) {
...
int ter_bionic_sigdelset(ter_bionic_sigset_t *set, int signal_number) {
  if (!set || signal_number < 1 || signal_number > 64) {
    errno = EINVAL;
    return -1;
  }
...
int ter_bionic_sigsuspend(const ter_bionic_sigset_t *set) {
  if (!set) { errno = EINVAL; return -1; }
  sigset_t host;
  ter_bionic_sigset_to_host(*set, &host);
