...
class Machine(object):
    ...
    # ------------------------------------------------------------ blocking
    def block_current(self, obj, note='', retry=False, deadline=0.0):
        ...
        to resume execution in the middle of a host function.  `deadline` is
        a time.monotonic() stamp after which the thread wakes by itself, the
        way a timed futex/condvar wait does on a real kernel.
        """
        t = self.current
        t.state = 'blocked'
        t.blocked_on = obj
    ...
    def expire_timers(self):
        """Wake every thread whose timed wait has run out.

        Baselib parks threads with a *timed* futex wait; without this they
        would sit blocked forever waiting for a wake that the kernel, not
        another thread, is supposed to deliver - which is exactly what kept
        the engine's loader thread (BootForm::Run) from ever finishing.
        """
        now = time.monotonic()
        woke = 0
        for t in self.threads:
            if t.state == 'blocked' and t.wake_at and now >= t.wake_at:
                t.state = 'ready'
                t.blocked_on = None
                t.wake_at = 0.0
                woke += 1
    ...
    def pump_threads(self, rounds=8):
        """Hand the CPU to every runnable guest thread, from host code.

        `yield_current` only works from inside a guest hook (it stops the
        emulator); between frames the player loop is in Python, so park the
        calling thread directly instead.  On the native build these are real
        pthreads and the kernel does this for us.
        """
        n = 0
