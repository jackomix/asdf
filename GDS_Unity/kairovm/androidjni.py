...
def _kutil_scale(j, this, a, sig):
    """Utility.getScaleRatio(reqW, reqH, dW, dH) -> percent, per classes.dex.

        w = display.getWidth()  + dW
        h = displayHeight       + dH        (minus the system bar, if any)
        return min(w * 100f / reqW, h * 100f / reqH)

    The result is a *percentage*: KairoPlugin divides by 100 again once it has
    folded in the app's own density factor.  Returning a plain 0..1 ratio here
    is what left `scaleRatio_` at 0 and `scaleRatioBase_` at +inf, which in
    turn collapsed every clip rect and vertex the UI produced.
    """
    reqw = int(a[0]) if len(a) > 0 else 0
    reqh = int(a[1]) if len(a) > 1 else 0
    dw = int(a[2]) if len(a) > 2 else 0
    dh = int(a[3]) if len(a) > 3 else 0
    w = j.h.width + dw
    hh = j.h.height + dh
    if not reqw or not reqh:
        return 100.0
    return min(w * 100.0 / reqw, hh * 100.0 / reqh)


# --------------------------------------------------------------- licensing
# classes.dex:
#   startLicenseCheck() news a LicenseChecker and kicks off an async doCheck().
#   getLicenseCheck()   -> -1 while the check is still running, otherwise
#                          0 when LicenseChecker.getResult() == 1 (ALLOW)
#                          and 1 for "not licensed".
#   googlePassCheck()   -> 2 as soon as BillingChecker fails to connect, which
#                          is exactly what a device with no Play services
#                          answers; only a live billing connection can return
#                          the real googlePassItem id.
# There is no Play store behind this port, so answer the way an owned,
# offline copy does: check finished, licensed, no Play Pass entitlement.
@jmethod('kairo/android/plugin/Utility', 'startLicenseCheck')
def _kutil_lic_start(j, this, a, sig):
    return None


@jmethod('kairo/android/plugin/Utility', 'getLicenseCheck')
def _kutil_lic_get(j, this, a, sig):
    return 0


@jmethod('kairo/android/plugin/Utility', 'googlePassCheck')
def _kutil_googlepass(j, this, a, sig):
    return 2
