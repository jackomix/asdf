...
class Bionic(object):
    ...
    # A timed futex wait is woken by the kernel, not by another thread, so
    # the VM has to run the timer itself.  The engine's loader thread parks
    # here (Baselib turns Thread.Sleep into a timed futex wait); without a
    # deadline it never woke and the game sat on its loading screen.  Real
    # time runs far slower than emulated frame time, so the exact interval
    # does not matter - only that the wait ends.
    FUTEX_MAX_WAIT = 0.05
    ...
    def _futex(self, uaddr, op, val, timeout=0):
        ...
        if cmd in (self.FUTEX_WAIT, self.FUTEX_WAIT_BITSET):
            ...
            deadline = 0.0
            if timeout:
                deadline = time.monotonic() + self.FUTEX_MAX_WAIT
            self.m.block_current(('futex', uaddr), deadline=deadline)
            return 0
