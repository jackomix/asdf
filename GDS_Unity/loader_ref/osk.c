 ...
 *
 * 0.95.9 v2 (user feedback on 0.95.8, cross-checked against the Xbox 2024
 * gamepad layout, Steam Deck Big Picture keyboard and Switch OSK):
 *   - MONOCHROME (grayscale) -- neutral over every Kairosoft game; layout/
 *     palette/font all still generated by tools/make_osk_font.py.
 *   - Smooth per-pixel panel gradient (was 4 strips), scrim 0.55 -> 0.68.
 *   - Function row snapped to the key grid (2u/2u/3u/1u/2u, 8px gaps both
 *     axes); common punctuation fills the bottom-right dead zone.
 *   - Symbols page = physical-keyboard shift PAIRS (small glyph above, big
 *     below like Deck/Xbox), so shift does something there; "#+="/"ABC"
 *     page key like Nintendo's.
 *   - Title-case labels, game title shown as sent (no ALL-CAPS), Book
 *     weight font (was Bold), natural space advance (was ~2.3x too wide).
 *   - L1/R1 move a real text caret (Xbox's LT/RB <>); insert/backspace at
 *     caret.  Page switch lives on the #+= key only.
 *   - Footer legend GONE: controller glyph badges drawn on the keys
 *     (B on Del, Y on Space, X on Shift, START pill on Done, L1/R1 pills
 *     on the text-box edges) -- user suggestion, matching Xbox/Deck.
 *   - Typing at max length shakes the n/max counter (error affordance).
 *   - SEL-cancel pill+label only appears when the PROMPT is cancellable:
 *     FepPanel.startInputPanel carries a negative_ label (dex-verified
 *     0.95.9); empty = the game offers no cancel (company-name boot
 *     prompt) and advertising SELECT there was a lie.
 ...
 *   L1/R1 text caret left/right   SELECT cancel START done
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
/* ------------------------------------------------------------ edit ops */
static int  g_shake;            /* >0: counter shake frames left (maxlen) */
...
static int insert_char(char c)
{
    ...
    if ((int)n >= g_maxlen) { g_shake = 14; return 0; }
...
enum { QB_TEXT, QB_SELT, QB_TITLE, QB_DIM, QB_BADGED, QB_BADGEL, QB_COUNT };
...
static void qb_reset(void)
{
    ...
    static const float cols[QB_COUNT][3] = {
        { OVKC_TEXT_C }, { OVKC_SEL_TEXT }, { OVKC_TITLE_C },
...
/* face-button circle with a dark letter (light disc) -- flips to a dark
 * disc + light letter when sitting on the selected (white) key. */
static void badge_circle(float cx, float cy, char letter, int on_selected)
{
    static const float face_l[3] = { OVKC_BADGE_FACE };
    static const float face_d[3] = { OVKC_FN_FACE };
    static const char ls[2] = { 0, 0 };
    char s[2] = { letter, 0 };
    disc(cx, cy, 8, on_selected ? face_d : face_l);
    float sc = 0.46f;
    float lw = ovk_textw(s, sc);
    int bucket = on_selected ? QB_BADGEL : QB_BADGED;
    (void)ls;
    ovk_emit(bucket, cx - lw * 0.5f,
             ovk_baseline_cell((int)(cy - 8), 16, sc), s, sc);
}

/* shoulder/start pill with tiny text */
                       ...
                       float tsc)
{
    ...
    ovk_emit(QB_BADGEL, x + (w - tw) * 0.5f,
...
{
    if (g_page == 0) {          /* letters -> symbols: keep (col,row) */
        int idx = g_sel;
        g_page = 1;
        if (idx >= OVK_N_LETTERS)
            g_sel = OVK_N_SYMBOLS + (idx - OVK_N_LETTERS);
        else if (idx / 10 >= 2)
            g_sel = OVK_N_SYMBOLS - 1;      /* lone `~ key */
        /* else same idx: rows 0-1 line up 1:1 */
    } else {
        int idx = g_sel;
        g_page = 0;
        if (idx >= OVK_N_SYMBOLS)
            g_sel = OVK_N_LETTERS + (idx - OVK_N_SYMBOLS);
        /* else same idx (`~ lands on 'a') */
    }
...
static void new_activate(const ovk_key_t *k)
{
    switch (k->act) {
    case OVKA_CHAR: {
        char c = g_shift ? k->hi : k->lo;   /* pairs on the symbols page */
...
/* ---------------------------------------------------------------- draw */
static void draw_key_label(const ovk_key_t *k, int selected)
{
    int bucket = selected ? QB_SELT : QB_TEXT;
    if (k->act == OVKA_CHAR) {
        char big = g_shift ? k->hi : k->lo;
        char s[2] = { big, 0 };
        float lsc = 0.85f;
        float lw = ovk_textw(s, lsc);
        ovk_emit(bucket, (float)k->x + ((float)k->w - lw) * 0.5f,
                 ovk_baseline_cell(k->y, k->h, lsc), s, lsc);
        /* physical pairing: small sibling glyph above (Deck/Xbox style) --
         * symbols page only; letters page flips the single glyph's case. */
        if (g_page == 1 && k->lo != k->hi) {
            char sm[2] = { (char)(g_shift ? k->lo : k->hi), 0 };
            float ssc = 0.45f;
            ovk_emit(bucket, (float)k->x + 6.0f,
        ...
        ovk_emit(bucket, (float)k->x + ((float)k->w - lw) * 0.5f,
                 ovk_baseline_cell(k->y, k->h, lsc), label, lsc);
    }
}
...
static void new_draw(void)
{
    ...
    /* scrim: dim the game behind (a bit darker after user feedback) */
    RRA(0, 0, 640, 480, 0.0f, 0.0f, 0.0f, 0.68f);
    ...
    /* title (as the game sent it -- no caps), n/max counter top-right */
    {
        char t[96];
        size_t i;
        ...
        ovk_emit(QB_TITLE, (float)tpos[0],
                 ovk_baseline_top((float)tpos[1], 0.75f), t, 0.75f);

        char cnt[16];
        snprintf(cnt, sizeof cnt, "%d/%d", (int)strlen(g_text), g_maxlen);
        float cw = ovk_textw(cnt, 0.58f);
        float off = 0.0f;
        if (g_shake > 0) {     /* error shake when typing past the limit */
            int amp = (g_shake * 5) / 14;
            off = (float)((g_shake & 1) ? -amp : amp);
            g_shake--;
        }
        ovk_emit(QB_DIM, 584.0f - cw + off,
                 ovk_baseline_top((float)OVK_COUNTER_Y, 0.58f), cnt, 0.58f);
    }
    ...
    /* keys */
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            static const float fs[3] = { OVKC_SHIFT_FACE };
            static const float fsel[3] = { OVKC_SEL_FACE };
            static const float esel[3] = { OVKC_SEL_EDGE };
            ...
            RR((float)k->x, (float)(k->y + k->h) - 2.0f, (float)k->w, 2.0f,
               OVKC_KEY_LO);
            draw_key_label(k, selected);
            /* controller badges on the keys they activate */
            if (k->act == OVKA_SHIFT)
                badge_circle((float)k->x + 13.0f, (float)k->y + 13.0f, 'X',
                             selected);
            else if (k->act == OVKA_SPACE)
                badge_circle((float)k->x + 13.0f, (float)k->y + 13.0f, 'Y',
                             selected);
            else if (k->act == OVKA_BKSP)
                badge_circle((float)k->x + 13.0f, (float)k->y + 13.0f, 'B',
                             selected);
            else if (k->act == OVKA_DONE)
    ...
    /* SEL-cancel affordance: only when the prompt offers a negative/cancel
     * (FepPanel negative_ label, dex-verified).  Bottom band, centered. */
    if (g_negative[0]) {
        const char *label = g_negative;
        float lsc = 0.44f;
        float tw = ovk_textw(label, lsc);
        float total = 24.0f + 4.0f + tw;
        float x0 = (float)px + ((float)pw - total) * 0.5f;
        float y0 = (float)(py + ph) - 14.0f;
        badge_pill(x0, y0, 24.0f, 13.0f, "SEL", 0.36f);
        ovk_emit(QB_DIM, x0 + 28.0f, ovk_baseline_top(y0 + 1.0f, lsc),
                 label, lsc);
    }
